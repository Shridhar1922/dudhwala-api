...
export class carModelRepository {
  ...
  async store(data: Object) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        console.log("Car makers", carMake);
  ...
  // find car model by car make id
  find({ id }) {
    return new Promise(async (resolve, reject) => {
      try {
        //let query = { isDeleted: false, make: id };
        const carRecord = await this.model
          .find({ make: id, isDeleted: false })
          .populate("make")
          .populate("features")
          .exec();
        console.log("carRecord", carRecord);
        if (carRecord) {
